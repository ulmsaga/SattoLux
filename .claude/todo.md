...
## Phase 5-1. 지난 결과 화면 (S05)

- [x] BE: `GET /api/result/history` — 주차별 이력 목록 API
  - 로그인 사용자 기준 전체 주차 목록, 최신순
  - 당첨 여부 / 최고 등수 / 등수별 건수 포함
- [x] BE: `GET /api/result/history/{year}/{month}/{week}` — 주차 상세 API
  - 당첨 번호 + 당첨 세트만 반환 (미당첨 세트 제외)
  - 각 세트의 일치 번호 포함
- [x] FE: `지난 결과` 메뉴 추가 (결과 비교 ~ 설정 사이)
- [x] FE: S05 목록 화면
  - 최신 주차순 리스트
  - 미당첨: 흐린 텍스트, 클릭 비활성
  - 당첨: 등수 배지 `[4등 1]` + `[상세>]` 버튼 활성
- [x] FE: S05 상세 화면 (신규 — S03 시각 패턴 차용)
  - 상단 뒤로가기 + 주차 제목
  - 당첨 번호 / 보너스 번호 표시
  - 당첨 세트만 표시, 일치 번호 강조 (S03 동일 패턴)
  - 세트 우측 등수 배지
